...
This document is the detailed reference for agents working in this repository. Start here after reading the root [`AGENTS.md`](../AGENTS.md).
...
- `packages`: shared packages (api-client, design-tokens, eslint-config, tsconfig).
...
Current code often returns anonymous JSON payloads like `{ message = "..." }` for validation/business errors. Follow the current code unless explicitly asked to standardize error handling globally.
...
- The login page is still a placeholder.
...
1. Inspect the closest existing slice in both frontend and backend.
2. Prefer extending current patterns over introducing target-state architecture early.
3. Keep changes small and end-to-end.
4. Generate migrations only through EF commands.
5. Run build/tests before finishing.
