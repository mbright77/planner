...
- Frontend auth handling now treats expired access tokens as signed-out when online, preserves cached protected views while offline, avoids serving IndexedDB fallback data for unauthorized responses, and clears auth-scoped query cache entries when a session is replaced or cleared
...
- [x] Finalize production release checklist
